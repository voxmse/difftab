...
public class OBJECT extends Hasher {
	...
	public void getHash(Object o, byte[] hash, int hashOffset) throws Exception {
		if(o==null){
			...
			hash[hashOffset + HASH_LENGTH] = COMPARE_AS_OBJECT_FOR_NULL;
		}else{
			String dataStr=o.toString();
			if(dataStr.isEmpty()){
				Arrays.fill(hash, hashOffset, hashOffset + HASH_LENGTH, (byte)0);
				hash[hashOffset + HASH_LENGTH] = COMPARE_AS_OBJECT_FOR_EMPTY;
			}else{
				md.update(dataStr.getBytes(StandardCharsets.UTF_8));
				md.digest(hash, hashOffset, HASH_LENGTH);
				hash[hashOffset + HASH_LENGTH] = COMPARE_AS_OBJECT;
			}
		}
	}
